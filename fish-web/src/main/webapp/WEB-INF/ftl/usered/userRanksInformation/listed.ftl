        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/usered/userRanksInformation/listed">入库列表</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="row">
	            <div class="col-sm-12">
	                ...
	                <form name="admin_list_sea" class="form-search" method="get" action="${basePath}/usered/userRanksInformation/listed">
		            ...
		            <table class="table table-bordered table-hover">
		                <thead>
                            <tr>
                                ...
                                <th style="text-align:center;">家乡</th>
                                <th style="text-align:center;">身份账号</th>
                                <th style="text-align:center;">从业时间</th>
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                ...
                                <td>{{d[i].professionalCategory}}</td>
                                <td>
                                    {{# if(d[i].type==0){ }}
                                    审核中
                                    {{# }else if(d[i].type==1){ }}
                                ...
                                <td>
                                <@shiro.hasPermission name="usered:userRanksInformation:remove">

                                    <button type="button" class="btn btn-primary  btn-xs" onclick="del('{{d[i].id}}')">删除</button>
                                </@shiro.hasPermission>
                                <@shiro.hasPermission name="usered:userRanksInformation:checked">
                                    <button type="button" class="btn btn-primary  btn-xs" onclick="check('{{d[i].id}}')"><i class="fa fa-paste"></i>入库审核</button>
                                </@shiro.hasPermission>
								</td>
		                    </tr>
		                {{# } }}
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		$.ajax({
			url: "${basePath}/usered/userRanksInformation/listed",
    ...
    function del(id){
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                url: "${basePath}/usered/userRanksInformation/remove",
                data:"id="+id,
                type: "POST",
                dataType:'json',
    ...
    function check(id){
        ...
        indexWin = layer.open({
            ...
            area: ['500px', '350px'],
            title:"审核",
            content:"${basePath}/usered/userRanksInformation/checked/"+id

        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/rotation/chart/list">轮播图列表</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="row">
	            <div class="col-sm-12">  
	                ...
	                <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/rotation/chart/list">
	        ...
	        <div class="example-wrap">
	            <div class="example">
				<@shiro.hasPermission name="rotation:chart:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="rotation:chart:delete">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
		                        ...
		                        <td>{{d[i].id}}</td>
							  	<td>{{d[i].name}}</td>
                                <td>
                                    <img src="{{d[i].images}}" style="height:80px" onerror="this.src='${basePath}/static/webupload/empty_mid.png'"/>
								</td>
                                <td>
                                    {{# if(d[i].type==1){ }}
                                    <span style="background-color: #dd9222;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">通知</span>
                                    {{# }else if(d[i].type==2){ }}
                                    <span style="background-color: #00ee00;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">公告</span>
                                    {{# }else if(d[i].type==3){ }}
                                    <span style="background-color: #0C0C0C;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">市场</span>
                                    {{# }else if(d[i].type==4){ }}
                                    <span style="background-color: #0d8ddb;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">政策</span>
                                    {{# }else if(d[i].type==5){ }}
                                    <span style="background-color: #ff0000;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">技术</span>
									{{# }else { }}
                                    <span style="background-color: #0000cc;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">管理</span>
                                    {{# } }}
                                </td>
								...
								<@shiro.hasPermission name="rotation:chart:edit">
                                    <a href="${basePath}/rotation/chart/edit/{{d[i].id}}"  class="btn btn-primary btn-xs">
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		...
		$.ajax({
			url: "${basePath}/rotation/chart/list",
	...
	function add(){
	    window.location.href="${basePath}/rotation/chart/add";

	}
    function edit(id){
        window.location.href="${basePath}/rotation/chart/edit/"+id;
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/rotation/chart/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',

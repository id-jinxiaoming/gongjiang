        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form class="form-horizontal m-t" id="myForm" action="${basePath}/ingenuity/usered/add" method="post" enctype="multipart/form-data">

                        <div class="form-group">
                            <label class="col-sm-3 control-label">工匠ID：</label>
                            <div class="col-sm-3">
                                <input id="useredId" type="text" class="form-control" name="useredId" required="" placeholder="请输入工匠ID" aria-required="true">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-8 col-sm-offset-3">
                                <input type="hidden" name="ingenuityId" value="${id}" />
                                <button class="btn btn-primary" type="submit">提交</button>
                                <a  class="btn btn-danger" type="button"  href="${basePath}/ingenuity/usered/list/${id}"><i class="fa fa-close"></i> 返回</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                success: function(data) { // data 保存提交后返回的数据，一般为 json 数据
                    ...
                    if(data.status == 200){
                        ...
                        layer.msg(data.message,{icon: 6,time:2000,shade: 0.1},function(index){
                            window.location.href="${basePath}/ingenuity/usered/list/${id}";
                        });
                    }else{

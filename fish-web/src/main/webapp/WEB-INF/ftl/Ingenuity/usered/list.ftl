...
<div class="wrapper wrapper-content animated fadeInRight">
	<div class="ibox float-e-margins">
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active">
                       <a href="${basePath}/ingenuity/usered/list/${worksId}">(${projectName})
                       </a>
                    </li>
                    <!--此id为匠心作品id-->
                </ul>
	    ...
	    <div class="ibox-content" style="border-color: transparent">
           ...
           <#-- <div class="row">
                <div class="col-sm-12">
                    <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/ingenuity/usered/list/${worksId}">
                ...
                <div class="col-sm-12">
                    ...
                    <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/ingenuity/usered/list/${worksId}">
                        <div class="col-sm-3" style="padding-right: 0px;padding-left: 0px;">
                            <div class="input-group">
                                <input type="text" id="key" class="form-control" name="key"  placeholder="输入需查询的工长名" />
                                <span class="input-group-btn">
	                                <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i> 搜索</button>
	                            </span>
                            </div>
	        ...
	        <div class="example-wrap">
	            <div class="example">
				<@shiro.hasPermission name="ingenuity:usered:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="ingenuity:usered:delete">
                    <button type="button" class="btn btn-outline btn-primary" onclick="DelAll()">批量删除</button>
				</@shiro.hasPermission>
                    <a href="${basePath}/ingenuity/works/list" style="float: right;"  class="btn btn-white">返回</a>
		            <table class="table table-bordered table-hover">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                ...
                                <td>{{d[i].id}}</td>
		                        <td>{{d[i].useredName}}</td>
                                <td>{{d[i].mobile}}</td>
                                <td>

								<@shiro.hasPermission name="ingenuity:usered:edit">
                                    <a href="${basePath}/ingenuity/usered/edit/${worksId}/{{d[i].id}}"  class="btn btn-primary btn-xs">
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		...
		$.ajax({
			url: "${basePath}/ingenuity/usered/list",
			data:{page:curr||1,id:id,key:key},
			type: "POST",
			dataType:'json',
	...
	function add(){
	    window.location.href="${basePath}/ingenuity/usered/add/${worksId}";
	}
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                url: "${basePath}/ingenuity/usered/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
